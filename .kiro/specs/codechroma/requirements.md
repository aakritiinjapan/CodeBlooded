...
1. WHEN a source code file is provided to the AST Analyzer, THE CodeChroma System SHALL parse the file into an Abstract Syntax Tree within 500 milliseconds for files under 1000 lines
2. WHEN the AST Analyzer processes a code file, THE CodeChroma System SHALL extract cyclomatic complexity values for each function and method
3. WHEN the AST Analyzer processes a code file, THE CodeChroma System SHALL extract lines of code metrics for each code block
4. WHEN the AST Analyzer processes a code file, THE CodeChroma System SHALL identify dependency relationships between code modules
5. WHERE TypeScript files are analyzed, THE CodeChroma System SHALL use the @typescript-eslint/parser for AST generation
...
5. WHEN the Audio Engine receives frequency mappings, THE CodeChroma System SHALL synthesize audio tones using Web Audio API with appropriate waveforms for each complexity level
...
2. WHEN the force-directed graph is rendered, THE CodeChroma System SHALL represent each code module as a node colored according to its complexity level
3. WHEN the force-directed graph is rendered, THE CodeChroma System SHALL display dependency relationships as edges connecting related nodes
...
4. WHEN the VS Code Extension is active, THE CodeChroma System SHALL display a webview panel containing the AST graph visualization
5. WHEN the VS Code Extension is active, THE CodeChroma System SHALL display a health score in the status bar representing overall file quality
...
1. WHEN the CLI Analyzer is invoked with a directory path, THE CodeChroma System SHALL recursively analyze all supported source files in that directory
...
4. WHEN the CLI Analyzer is invoked with a complexity threshold parameter, THE CodeChroma System SHALL exit with a non-zero status code if any file exceeds the threshold
...
3. WHEN an unsupported file type is provided, THE CodeChroma System SHALL return an error message indicating the file type is not supported
4. THE AST Analyzer SHALL provide a plugin architecture allowing additional language parsers to be registered
5. WHERE a language parser is registered, THE CodeChroma System SHALL use the appropriate parser based on file extension
...
1. THE CodeChroma System SHALL organize code into separate packages for core, lsp-server, vscode-extension, and cli-analyzer
2. THE CodeChroma System SHALL place shared AST analysis functionality in the core package
3. THE CodeChroma System SHALL place shared sensory mapping functionality in the core package
4. THE CodeChroma System SHALL place shared audio engine functionality in the core package
5. THE CodeChroma System SHALL place shared visualization functionality in the core package
...
5. THE LSP Server SHALL utilize the core CodeChroma System components for all analysis operations
